...
{
    public class ClientItemInfoCsv : IAssetDeserializer<ClientItemInfoAsset>
    {
        private static readonly ILogger Logger = LogProvider.Logger<Logger>(typeof(QuestScheduleIdCsv));

        private readonly ClientItemInfoCsvReader rowReader = new();

        public ClientItemInfoAsset ReadPath(string path)
        {
            var asset = new ClientItemInfoAsset();

            rowReader.ReadPath(path)
                .ForEach(item =>
                {
                    asset.Add(item.ItemId, item);
                });

            return asset;
        }

        public class ClientItemInfoCsvReader : CsvReaderWriter<ClientItemInfo>
        {
