...
public class ServerCommand : ICommand
{
    ...
    private MetricsCollector<DdonServerMetricsSnapshot> _loginServerMetricsCollector;
    private DdonGameServer _gameServer;
    private MetricsCollector<DdonServerMetricsSnapshot> _gameServerMetricsCollector;
    ...
    public CommandResultType Run(CommandParameter parameter)
    {
        ...
        if (_loginServer == null)
        {
            _loginServer = new DdonLoginServer(_setting.LoginServerSetting,
                _serverScriptManager.GameServerSettingsModule.GameSettings, _database, _assetRepository);

            if (_setting.LoginMetricSettings.Enabled)
            {
        ...
        if (_gameServer == null)
        {
            ...
            if (_setting.GameMetricSettings.Enabled)
            {
        ...
        if (parameter.Arguments.Contains("start"))
        {
            ...
            _gameServerMetricsCollector?.Start("GameServerMetrics");
        ...
        {
            _loginServerMetricsCollector?.Stop();
            _gameServerMetricsCollector?.Stop();
            _webServer.Stop();
            _gameServer.Stop();
            _loginServer.Stop();
            _database.Stop();
