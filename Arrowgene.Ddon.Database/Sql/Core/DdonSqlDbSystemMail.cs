...
public partial class DdonSqlDb : SqlDb
{
    ...
    public override long InsertSystemMailMessage(MailMessage message)
    {
        using DbConnection connection = OpenNewConnection();
        return InsertSystemMailMessage(connection, message);
    }

    public override long InsertSystemMailMessage(DbConnection connection, MailMessage message)
    {
    ...
    public override List<MailMessage> SelectSystemMailMessages(uint characterId)
    {
        using DbConnection connection = OpenNewConnection();
        return SelectSystemMailMessages(connection, characterId);
    }

    public List<MailMessage> SelectSystemMailMessages(DbConnection conn, uint characterId)
    {
        ...
        ExecuteInTransaction(conn =>
        {
            ExecuteReader(conn, SqlSelectSystemMailMessages,
                command => { AddParameter(command, "@character_id", characterId); }, reader =>
                {
                    while (reader.Read())
                    {
                        MailMessage result = ReadSystemMailMessage(reader);
                        results.Add(result);
                    }
                });
        });
    ...
    public override MailMessage SelectSystemMailMessage(ulong messageId)
    {
        using DbConnection connection = OpenNewConnection();
        return SelectSystemMailMessage(connection, messageId);
    }

    public MailMessage SelectSystemMailMessage(DbConnection conn, ulong messageId)
    {
    ...
    private MailMessage ReadSystemMailMessage(DbDataReader reader)
    {
        MailMessage obj = new();
