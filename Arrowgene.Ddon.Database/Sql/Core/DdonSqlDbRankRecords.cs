...
public partial class DdonSqlDb : SqlDb
{
    ...
    private static readonly string[] RankRecordFields = new[]
    {
        /* record_id */ "character_id", "quest_id", "score", "date"
    };

    private static readonly string SqlInsertRankRecord =
        $"INSERT INTO \"{RankRecordTableName}\" ({BuildQueryField(RankRecordFields)}) VALUES ({BuildQueryInsert(RankRecordFields)});";

    private static readonly string SqlDeleteAllRankRecords = $"DELETE FROM {RankRecordTableName};";
    ...
    public override bool InsertRankRecord(uint characterId, uint questId, long score, DbConnection? connectionIn = null)
    {
    ...
    public override List<uint> SelectUsedRankingBoardQuests(DbConnection? connectionIn = null)
    {
        return ExecuteQuerySafe(connectionIn, connection =>
        {
            List<uint> list = new();
            ExecuteReader(connection,
                ...
                reader =>
                {
                    while (reader.Read()) list.Add(GetUInt32(reader, "quest_id"));
                });
    ...
    public override List<CDataRankingData> SelectRankingDataByCharacterId(uint characterId, uint questId, uint limit = 1000, DbConnection? connectionIn = null)
    {
        List<CDataRankingData> list = ExecuteQuerySafe(connectionIn, connection =>
        {
            List<CDataRankingData> list = new();
            ExecuteReader(connection,
    ...
    public override List<CDataRankingData> SelectRankingData(uint questId, uint limit = 1000, DbConnection? connectionIn = null)
    {
        List<CDataRankingData> list = ExecuteQuerySafe(connectionIn, connection =>
        {
            List<CDataRankingData> list = new();
    ...
    public override bool DeleteAllRankRecords(DbConnection? connectionIn = null)
    {
        return ExecuteQuerySafe(connectionIn, connection =>
        {
            return ExecuteNonQuery(
